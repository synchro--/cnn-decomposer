...
def VBMF(Y, cacb, sigma2=None, H=None):
    ...
    if sigma2 is None: 
        ...
        sigma2 = sigma2_opt.x
        log.debug("Estimated sigma2: %s", sigma2)
...
def VBsigma2(sigma2,L,M,cacb,s,residual):
    """Compute the VBMF free-energy objective for noise variance.

    Parameters
    ----------
    sigma2 : float
        Candidate noise variance.
    L : int
        Number of rows in the input matrix.
    M : int
        Number of columns in the input matrix.
    cacb : float
        Product of prior variances.
    s : numpy.ndarray
        Singular values used by the objective.
    residual : float
        Residual energy outside the truncated rank.

    Returns
    -------
    float
        Free-energy objective value.
    """
    H = len(s)
...
def EVBsigma2(sigma2,L,M,s,residual,xubar):
    """Compute the empirical VBMF objective for noise variance.
    ...
    residual : float
        Residual energy outside the truncated rank.
    xubar : float
        Empirical VBMF threshold parameter.

    Returns
    -------
    float
        Objective value for ``sigma2``.
    """
    H = len(s)
...
def phi0(x):
    """Return the VBMF ``phi0`` helper term.
...
def phi1(x, alpha):
    """Return the VBMF ``phi1`` helper term.
